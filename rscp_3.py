...
class rover_broker():
    ...
    def rover_on_send(self):
        ...
        self.frame=sender.armdis_frame()
        self.ser1.write(self.frame)
        self.frame=sender.ack_frame()
        self.ser1.write(self.frame)
        self.frame=sender.dedect_frame()
        self.ser1.write(self.frame)
        self.frame=sender.taskfinshed_frame()
        self.ser1.write(self.frame)
        self.frame=sender.navigation_to_gps_frame()
        self.ser1.write(self.frame)
        

    def main(self):
        # self.ser = serial.Serial("/dev/pts/4", 9600, timeout=None)
        self.ser1 = serial.Serial("/dev/pts/11", 115200, timeout=None)
